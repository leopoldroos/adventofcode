...
console.log('_________________SERVER?')

const controllers = {
  spiral: require('./controllers/spiral'),
}
...
app.get('/generic/:controller', (req, res) => {
  try {
    ...
    const data = controller
      ? controller.run(req)
      : 'Gör en automatisk lista på tillgänglig controllers här...' // TODO
    res.send(data)
  } catch (err) {
...
app.get('/files/:filename', (req, res) => {
  fs.readFile(
    path.resolve(__dirname, './', `server/files/${req.params.filename}`),
    { encoding: 'utf-8' },
    (err, data) => {
      if (err) {
        console.log('path.resolve error: ', err)
