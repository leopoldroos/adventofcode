...
const url = {
  buildQuery: function (params) {
    return Object.keys(params)
      .map(k => encodeURIComponent(k) + '=' + (params[k] !== undefined && params[k] !== null ? encodeURIComponent(params[k]) : ''))
      .join('&')
  }
}
...
function sendRequest (method, baseUrl, path, options = {}) {
  ...
  if (options.queryParams) {
    u += (u.indexOf('?') === -1 ? '?' : '&') + url.buildQuery(options.queryParams)
  }
  let fetchOptions = {
    method: method || 'get',
    headers: {
  ...
  return window.fetch(u, fetchOptions).then(res => {
